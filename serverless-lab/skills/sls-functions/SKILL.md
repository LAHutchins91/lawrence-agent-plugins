...
name: sls-functions
description: >
  List Serverless Framework functions (name + handler) and event hints
  (http / httpApi / schedule / sns / sqs / stream / s3 / websocket / alb /
  eventBridge) from pasted serverless.yml / serverless.ts. Local only —
  never runs Serverless Framework CLI or AWS deploy, no fetch.
version: 1.0.0
